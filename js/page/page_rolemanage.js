                ...
                "columns": [
                    ...
                    { "data": "remark" }, {
                        ...
                        "defaultContent": ['<a href="javascript:;" name="edit" class="btn btn-primary">编辑</a>',
                            ...
                            ''/// 删除功能取消了<a href="javascript:;" class="btn btn-danger" name="delete">删除</a> 
                            ].join('')
                    }
                ],
                "order": [
                                    	...
                                    	if (json.success) {
                                            for (var i = 0; i < json.data.length; i++) {
                                                var Str = '<tr class="roleManger">' +
                                                    '<td style="width:360px;">' + json.data[i].moduleName + '</td>' +
                                                    '<td>' +
                                                ...
                                                buttons: {
                                                    success: {
                                                        ...
                                                        callback: function () {
                                                            ...
                                                            alert(roleId+":"+roleId+"::moduleIds="+moduleIds);
...
jQuery(document).ready(function () {
    ...
    if (roleForm.length) {
        var d = FormValidation.init();

        if (!btn.prop('disabled')) {
            roleForm.on('submit', function (e) {
                e.preventDefault;
                console.log(roleForm.valid());
                if (!roleForm.valid()) {
                    return
                }
                btn.prop('disabled', true);
                var data = roleForm.serialize();
                Util.setAjax(Inter.getApiUrl().addRole+"?"+data, data, function (json) {
                    if (json.success) {
                        btn.prop('disabled', false);
                        window.location.href = '/backStage/role/rolemanage'
                    } else {
